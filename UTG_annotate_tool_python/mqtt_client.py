        ...
        if msg.topic == "textTopic":
            ...
            if msg.topic == "tempScreenTopic":
                ...
                if similarity<=0.8:
                    ...
                    images = [os.path.join(dir_path, str(screenNum)+"_screenshot.jpg"),image_path]
                    isSame = same_screen_discriminator(images) #相同界面判断，human evaluator介入的touch point
            ...
            if msg.topic == "stepOperateAgent":
                print("stepOperateAgent")
                # 解析JSON字符串
                x = None
                y = None
                query_data = json.loads(msg.payload.decode())
                ...
                call_back_data = {"x": x, "y": y}
                call_back_message = json.dumps(call_back_data)

                if(client.publish("stepOperateVM", call_back_message)):
                    print("回传成功")
                else:
                    print("回传失败")

            if msg.topic == "previewVM":
                #暂存下一步screen,并结合previewAgent的screen生成教程gif
                print("previewVM")
                # 解析JSON字符串
                image_path = None
                question = None
                ...
                if (question=="preStep"):
                    previewQuery["preStep"] = image_path
                    previewQuery["x"] = x
                    previewQuery["y"] = y
                    print("previewQuery", previewQuery)
                elif (question=="postStep" and x==previewQuery["x"] and y==previewQuery["y"]):
                    previewQuery["postStep"] = image_path
                    print("previewQuery", previewQuery)
                    generate_step_preview(before_img_path=previewQuery["preStep"], after_img_path=previewQuery["postStep"], click_video_path="tap.mov",output_gif_path="ui_demo.gif",click_position=(previewQuery["x"],previewQuery["y"]),bg_threshold=230,transition_type="slide", video_scale=0.8, duration=40, transition_frames=10, final_fram_num=100, output_size=(1080, 2280))
                    time.sleep(0.5)
                    with open("ui_demo.gif", "rb") as f:
                        gif_data = base64.b64encode(f.read()).decode('utf-8')
                    if(client.publish("gif_channel", gif_data)):
                        print("回传GIF成功")
                        previewQuery["preStep"] = None
                        previewQuery["postStep"] = None
                        previewQuery["x"] = None
                        previewQuery["y"] = None
                    else:
                        print("回传GIF失败")

            if msg.topic == "previewAgent":
                print("previewAgent")
                with open("ui_demo.gif", "rb") as f:
                    gif_data = base64.b64encode(f.read()).decode('utf-8')
                if(client.publish("gif_channel", gif_data)):
                ...
                if(client.publish("stepPreview", call_back_message)):
                    print("回传成功")
                else:
                    print("回传失败")
                '''
                ...
                '''
                if isInitialQuery:
                    print("generate static tutorial")
                    package_name = "ctrip.android.view"
                    dir_path = os.path.join(os.getcwd(), 'UIdata', package_name)
                    ...
                    time.sleep(3)
                    resize_image("tutorial_final.png", "tutorial_final.jpg")
                    with open("tutorial_final.jpg", 'rb') as f:
                        image_data = base64.b64encode(f.read()).decode('utf-8')
                    if(client.publish("cloudImage", image_data)):
                        print("回传成功")
                    else:
                        print("回传失败")
            ...
            '''
            if msg.topic == "imageTopic":
                image_path = handle_image(msg)
                if image_path != None and question != None:
                    position, instruction = generate_with_GPT(image_path, question)
                    print(position)
                    print(instruction)
                    image_path = None
                    question = None
            elif msg.topic == "textTopic":
                question = handle_text(msg)
                if image_path != None and question != None:
                    position, instruction = generate_with_GPT(image_path, question)
                    print(position)
                    print(instruction)
            ...
            # if (msg.topic == "imageTopic" or msg.topic == "textTopic") and (image_path != None and question != None):
            '''
